...
- **授权执行**：机械安装步骤（见"机械安装计划与授权执行"）必须先展示计划并获得用户明确确认后才执行；禁止用户无感的自主安装与覆盖安装。
...
artifact-chain-setup 不从退出码推断故障类型，只要 stdout 能解析为 JSON，就必须读取 `nativeBinding`
...
- **先计划后询问**：把计划以"步骤 + 确切命令/patch 内容 + 目标文件"的形式展示给用户，等待明确确认；未获确认不得写入任何内容。
- **禁止自主安装与覆盖安装**：必须先取得用户明确确认，再执行计划中的写入。

...
test('INSTALL.md contains precise npm install command', async () => {
  ...
  assert.match(content, /pnpm add -D artifact-graph@0\.4\.0/);
});

test('INSTALL.md contains GitHub fallback with precise tag', async () => {
  const content = await readUtf8(INSTALL_PATH);
  assert.match(content, /github:mzdbxqh\/artifact-graph#artifact-graph-v0\.4\.0/);
});
...
test('English README contains version 0.4.0 reference', async () => {
...
test('Chinese README contains version 0.4.0 reference', async () => {
...
test('Codex JSON root discovery extracts versioned cache root from fixture JSON', async () => {
  ...
  assert.equal(
    versionedRoot,
    '/home/user/.codex/plugins/cache/artifact-chain-assistant/artifact-chain-assistant/0.4.0',
  );
});
...
test('Claude JSON root discovery extracts installPath from fixture JSON', async () => {
  ...
  const fixtureJson = JSON.stringify([
    {
      ...
      scope: 'user',
      enabled: true,
      installPath: '/home/user/.claude/plugins/cache/artifact-chain-assistant/artifact-chain-assistant/0.4.0',
    },
  ]);
  ...
  assert.equal(plugin.enabled, true);
  assert.match(plugin.installPath, /artifact-chain-assistant\/0\.4\.0$/);
});
...
`;
  ...
  try {
    ...
    assert.equal(
      result,
      '/home/Test User/.codex/plugins/cache/artifact-chain-assistant/artifact-chain-assistant/0.4.0',
...
`;
  ...
  try {
    ...
    assert.equal(
      result,
      '/home/user/.claude/plugins/cache/artifact-chain-assistant/artifact-chain-assistant/0.4.0',

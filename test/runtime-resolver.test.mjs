 ...
 */
async function createFakeCli(dir, { version = '0.4.0', executable = true } = {}) {
 ...
 */
async function createBareCli(dir, { version = '0.4.0', executable = true } = {}) {
...
test('prefers project-local .bin over PATH', async () => {
  ...
  try {
    ...
    await createFakeCli(projectRoot, { version: '0.4.0' });
    ...
    assert.equal(result.actualVersion, '0.4.0');
...
test('prefers PATH over ARTIFACT_GRAPH_LEGACY_CLI', async () => {
  ...
  try {
    ...
    await createFakeCli(pathDir, { version: '0.4.0' });
...
test('falls back to ARTIFACT_GRAPH_LEGACY_CLI when PATH has no match', async () => {
  ...
  try {
    await mkdir(join(projectRoot, 'node_modules'), { recursive: true });
    await createFakeCli(legacyDir, { version: '0.4.0' });
    const legacyCli = join(legacyDir, 'node_modules', '.bin', 'artifact-graph');
...
test('rejects non-executable file even if it exists', async () => {
  ...
  try {
    ...
    await createFakeCli(pathDir, { version: '0.4.0', executable: false });
...
test('resolves executable file when it has correct permissions', async () => {
  ...
  try {
    await mkdir(join(projectRoot, 'node_modules'), { recursive: true });
    await createFakeCli(pathDir, { version: '0.4.0', executable: true });
...
test('uses platform PATH delimiter', async () => {
  ...
  try {
    await createBareCli(pathDir, { version: '0.4.0' });

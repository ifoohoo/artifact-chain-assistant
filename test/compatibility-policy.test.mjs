...
const VALID_POLICY = {
  ...
  pluginVersion: '0.4.1',
  artifactGraph: {
    verifiedVersion: '0.4.1',
    installSpec: 'artifact-graph@0.4.1',
    githubFallbackSpec: 'github:mzdbxqh/artifact-graph#artifact-graph-v0.4.1',
  },
};
...
async function createFixture(overrides = {}) {
  ...
  const pv = overrides.pluginVersion ?? '0.4.1';
...
test('loadCompatibilityPolicy loads and freezes valid policy', async () => {
  ...
  try {
    ...
    assert.equal(policy.pluginVersion, '0.4.1');
    assert.equal(policy.artifactGraph.verifiedVersion, '0.4.1');
    assert.equal(policy.artifactGraph.installSpec, 'artifact-graph@0.4.1');
...
test('validateCompatibilityPolicy detects non-precise installSpec', async () => {
  const { validateCompatibilityPolicy } = await import('../scripts/lib/compatibility-policy.mjs');
  const dir = await createFixture({
    artifactGraph: { installSpec: 'artifact-graph@^0.4.1' },
  });
  try {
    const issues = await validateCompatibilityPolicy(dir);
...
const FORBIDDEN_INSTALL_SPECS = [
  ...
  { cmd: 'pnpm add -D artifact-graph@^0.4.1', forbidden: true, label: 'pnpm add -D artifact-graph@^0.4.1' },
  { cmd: 'pnpm add artifact-graph@^0.4.1', forbidden: true, label: 'pnpm add artifact-graph@^0.4.1' },
  ...
  { cmd: 'npm install -D artifact-graph@^0.4.1', forbidden: true, label: 'npm install -D artifact-graph@^0.4.1' },
  { cmd: 'npm install artifact-graph@^0.4.1', forbidden: true, label: 'npm install artifact-graph@^0.4.1' },
  { cmd: 'npm i -D artifact-graph@^0.4.1', forbidden: true, label: 'npm i -D artifact-graph@^0.4.1' },
  { cmd: 'npm i artifact-graph@^0.4.1', forbidden: true, label: 'npm i artifact-graph@^0.4.1' },
  // ── GitHub spec ──
  ...
  { cmd: 'pnpm add artifact-graph@~0.4.1', forbidden: true, label: 'pnpm add artifact-graph@~0.4.1 (tilde range)' },
  ...
  { cmd: 'pnpm add artifact-graph@^0.4.1 --save-dev', forbidden: true, label: 'pnpm add artifact-graph@^0.4.1 --save-dev (spec+caret before flag)' },
  ...
  { cmd: 'npm install artifact-graph@0.4.1 -D', forbidden: false, label: 'npm install artifact-graph@0.4.1 -D (pinned, spec before flag)' },
  { cmd: 'pnpm add --save-dev github:mzdbxqh/artifact-graph#artifact-graph-v0.4.1', forbidden: false, label: 'pnpm add --save-dev github:mzdbxqh/artifact-graph#artifact-graph-v0.4.1 (tagged github)' },
  // ── Allowed: pinned version (original order) ──
  { cmd: 'pnpm add -D artifact-graph@0.4.1', forbidden: false, label: 'pnpm add -D artifact-graph@0.4.1 (pinned)' },
  { cmd: 'npm install -D artifact-graph@0.4.1', forbidden: false, label: 'npm install -D artifact-graph@0.4.1 (pinned)' },
];

...
async function createFakeProject({ version = '0.3.1', withBin = true, pkgName = 'artifact-graph' } = {}) {
...
test('inspectArtifactGraphRuntime detects version mismatch', async () => {
  ...
  try {
    ...
    assert.equal(result.expectedVersion, '0.3.1');
    assert.match(result.remediation, /pnpm add -D artifact-graph@0\.3\.1/);
  } finally {
    await rm(projectRoot, { recursive: true, force: true });
  }
});
...
test('inspectArtifactGraphRuntime detects unresolvable version (wrong package name)', async () => {
  const { inspectArtifactGraphRuntime } = await import(RUNTIME_MODULE);
  const projectRoot = await createFakeProject({ version: '0.3.1', pkgName: 'wrong-package' });
  try {
...
const arg = process.argv[2];
if (arg === '--version') { process.stdout.write('0.3.1\\n'); process.exit(0); }
process.exit(7);
`);

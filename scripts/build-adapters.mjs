...
// Root plugin manifests required by release-skill assess. They are deterministic copies of the
// generated adapter manifests above, so the adapter template render stays the single version source.
// The marketplace plugin source still points at ./adapters/claude and ./adapters/codex; these root
// files are release metadata only, not an alternate install root and not a host installation surface.
const rootManifestCopies = [
  { from: 'adapters/claude/.claude-plugin/plugin.json', path: '.claude-plugin/plugin.json' },
  { from: 'adapters/codex/.codex-plugin/plugin.json', path: '.codex-plugin/plugin.json' },
];
for (const copy of rootManifestCopies) {
  const target = join(root, copy.path);
  const content =
    generatedContent.get(copy.from) ?? (await readFile(join(root, copy.from), 'utf-8'));
  if (check) {
    let current = '';
    try {
      current = await readFile(target, 'utf-8');
    ...
    if (current !== content) {
      drift = true;
      console.error(`Generated root manifest drift: ${copy.path}`);
    }
  } else {
    await mkdir(dirname(target), { recursive: true });
    await writeFile(target, content);

...
const kimiCatalogPath = join(root, 'adapters', 'kimi', 'agent-methods', 'catalog.yaml');
...
function runRegistryExpectFail(args) {
  ...
  assert(existsSync(kimiCatalogPath), 'kimi adapter should have agent-methods/catalog.yaml');
  assert(
  ...
  assert(
    readFileSync(kimiCatalogPath, 'utf-8') === rootCatalogContent,
    'kimi adapter catalog should match root catalog',
  );
  ...
  const kimiValidation = runRegistry(['validate', '--catalog', kimiCatalogPath]);
  assert(kimiValidation.ok === true, 'kimi adapter catalog schema validation should pass');
  ...
  // ── 3. Exactly 16 entries ──
  assert(index.entries.length === 16, `expected 16 entries, got ${index.entries.length}`);
  const operationRefs = new Set(['artifact.help', 'artifact.setup', 'artifact.quickstart']);
  for (const entry of index.entries) {
    if (operationRefs.has(entry.ref)) {
  ...
  // NOTE: no kimi resolve section — the agent-method-registry CLI currently only
  // accepts '--host codex' and '--host claude-code'. Kimi adapter coverage is the
  // catalog existence/consistency/schema checks above; resolve verification for
  // kimi-code requires upstream registry support first.
...
if (issues.length > 0) {
  ...
  console.log('Method registry check passed: 16 entries (13 workflow + 3 operation), schema/query/resolve/overlay all verified.');
}

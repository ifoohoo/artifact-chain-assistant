...
- **沿用精确授权**：把当前任务已明确授权的目标和动作交给下游技能，不要求用户重复确认；下游发现范围扩大时再请求授权。
...
- 无领域限定的裸全局输入（"hello"、通用能力查询、"怎么开始"）不在本表范围，
  本技能不认领，交由宿主或其他插件处理。
...
**匹配关键词**：拆分制品、拆分 PRD、制品重组、跨文件移动、重编号、restructure artifact

**路由目标**：`artifact-chain-restructure`

**理由**：该技能先用 `artifact-graph restructure inspect` 区分确定性变换与业务语义判断，再编译实际候选；只有发生业务语义判断时才安排独立只读复审。完整人工映射和纯确定性路径不强制调用模型。用户授权覆盖真实写入、复审通过且操作者给出写入前提后，同一技能才继续路由应用、恢复与清理。

**最小提示词**：`请在我授权的路径内分析这次制品重组，生成映射和候选计划；如果我的授权已经覆盖应用，请先把计划保存到候选写集之外的路径并告诉我该路径，再按需要确认的写入前提执行。`
...
**匹配关键词**：技能族规范、规范采用、职责边界、治理检查、配置就绪、专业审计

**路由目标**：先用 `artifact-chain-where-am-i` 确认项目事实和规范适用性；需要环境诊断时转 `artifact-chain-setup`，需要经授权写入项目配置时转 `artifact-chain-bootstrap`，需要通用图健康结果时转 `artifact-audit`

**理由**：Audit 定义技能族制品规范，助手协助采用，`artifact-graph` 提供通用图检查。配置可读不等于专业规范通过；规范来源缺失、不可读或公共合同尚未发布时，只暂停相应专业判断，不把目标判为违规。

...
> **安全的第一步：** 安装后先使用只读的 `artifact-chain-help` 技能查看 Family API 和采用
> 步骤；在授权初始化或维护写入前，先用 `where-am-i` 做只读项目分诊。
...
> `artifact-chain-assistant` 0.8.0，上述安装命令才能生效。
...
1. 安装插件 0.8.0（见上文）和运行时：`pnpm add -D artifact-graph@0.8.0`。

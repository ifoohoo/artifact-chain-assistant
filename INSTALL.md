...
- Node.js `>=22.22.2 <23`.
- `artifact-graph` 0.14.0 installed in the target project.
...
```bash
pnpm add -D artifact-graph@0.14.0
```

If the npm registry is unavailable, use the explicit GitHub fallback pinned to the verified tag:

```bash
pnpm add -D github:ifoohoo/artifact-graph#artifact-graph-v0.14.0
```

> **Never** install with an unlocked range (`artifact-graph`, `artifact-graph@latest`,
> `artifact-graph@^0.14.0`) or an unpinned GitHub URL (`github:ifoohoo/artifact-graph`).
...
command (`pnpm add -D artifact-graph@0.14.0`) and exits non-zero.
...
1. **Install the CLI** — `pnpm add -D artifact-graph@0.14.0` (see Prerequisites above). The
...
```bash
# 1. Install dependencies from lockfile (gets artifact-graph@0.14.0)
...
`artifact-graph@0.14.0` and the plugin marketplace repository on an internal registry. The mirror

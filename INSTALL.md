...
- Node.js `>=22.0.0`.
- `artifact-graph` 0.8.1 installed in the target project.
...
| --- | --- | --- |
| `artifact-chain-assistant` 0.8.1 | `artifact-graph` 0.8.1 | `pnpm add -D artifact-graph@0.8.1` |
...
```bash
pnpm add -D artifact-graph@0.8.1
```

If the npm registry is unavailable, use the explicit GitHub fallback pinned to the verified tag:

```bash
pnpm add -D github:ifoohoo/artifact-graph#artifact-graph-v0.8.1
```

> **Never** install with an unlocked range (`artifact-graph`, `artifact-graph@latest`,
> `artifact-graph@^0.8.1`) or an unpinned GitHub URL (`github:ifoohoo/artifact-graph`).
...
command (`pnpm add -D artifact-graph@0.8.1`) and exits non-zero.
...
> must publish and enable `artifact-chain-assistant` 0.8.1 before the install commands above will
> succeed.
...
1. **Install the CLI** — `pnpm add -D artifact-graph@0.8.1` (see Prerequisites above).
...
```bash
# 1. Install dependencies from lockfile (gets artifact-graph@0.8.1)
...
`artifact-graph@0.8.1` and the plugin marketplace repository on an internal registry. The mirror

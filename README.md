...
the Codex and Claude Code adapter roots. Validate with `check-workflow-profile.mjs` before
...
```bash
# Runtime (required)
npm install --save-dev artifact-graph@0.8.5
```
...
1. Install plugin 0.8.5 (above) and runtime: `pnpm add -D artifact-graph@0.8.5`.
